...
- 全量自动化测试：507 个测试运行，0 failure，0 error，2 skipped。
...
面向 GitHub Pull Request 的 AI Review 后端系统，负责审查任务编排、异步执行、规则召回、模型审查、结果落库和 GitHub 评论回写。通过 Redis + headSha 去重避免重复审查，使用 RabbitMQ 解耦 Webhook 触发和耗时审查流程；在 LLM 前强制执行 SQL 风险、敏感信息、测试缺失等确定性规则，并通过 patch verification / location guard 过滤无法绑定变更证据的问题。完成离线评估与运行态验证，507 个自动化用例通过，离线评估 Precision 85.71%、Recall 100%。
...
- 建立离线评估和运行验证基线，覆盖 SQL 风险、硬编码密钥、prompt injection、缺少测试、API 契约变化和非法 JSON 降级等场景；507 个自动化用例通过，离线评估 Precision 85.71%、Recall 100%。
...
有三类证据：第一，项目接入了 GitHub Webhook、API Key 鉴权、RabbitMQ、PostgreSQL/pgvector、Redis 和 GitHub 评论回写这些真实后端组件；第二，单测覆盖 507 个用例，包含 Webhook、任务状态、评论、规则、RAG、LLM parser、fix 命令等模块；第三，运行态 smoke 在 Docker 环境下验证了鉴权、Webhook 签名、任务创建、Redis 去重、数据库落库和 MQ/DLQ 链路。
