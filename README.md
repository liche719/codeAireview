...
6. 输出结构化 JSON 审查结果；
7. 将 review issue 入库；
8. 生成 Markdown 报告并回写到 GitHub PR 顶部评论。
...
- GitHub PR 顶部评论回写。
- 评论策略采用追加式：每次成功审查都会创建新的 top-level comment，保留历史，不更新旧评论。
...
```text
1. Webhook opened / synchronize / reopened
2. 签名校验 + Redis 去重
...
- `CODEPILOT_GITHUB_COMMENT_ENABLED`：是否启用 GitHub PR 评论回写。
...
审查成功后，系统会生成 Markdown 审查报告并回写到 GitHub PR 顶部评论。

当前策略是：

- 每次成功审查都创建新的 top-level comment；
- 不更新旧评论；
- 保留完整审查历史；
- 评论 body 中会带有 `<!-- codepilot-ai-review:liche719/codeAireview -->` marker，方便后续识别。
